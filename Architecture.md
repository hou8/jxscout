...
5.  **网络交互与正则模式**
    ...
    *   **正则表达式执行**：审计 `.match()` / `.exec()` / `.test()` 的使用。[regex-match.ts](file:///home/hou8/codebase/edc-mind/vendors/jxscout/pkg/ast-analyzer/tree-analyzers/regex-match.ts)
...
### 3.8 NATS 消息发布 (NATS Publisher)
订阅数据库事件总线上的 AST 分析完成事件，在分析结果落库后，将分析结果整理并推送到指定的 NATS 消息队列。
*   **代码映射**：[module.go](file:///home/hou8/codebase/edc-mind/vendors/jxscout/internal/modules/nats/module.go)
*   **配置参数**：
    *   `-nats-enabled` (默认 `true`)：是否启用 NATS 推送。
    *   `-nats-url` (默认 `nats://localhost:4222`)：NATS 服务端的连接 URL。
*   **发布主题 (Subject)**：`jxscout.ast.analysis`
*   **消息 Payload 结构**：
    *   为了防止冗余，在 Payload 结构中进行了数据整理：内外层路径字段（`filePath`）进行了合并去重（仅在最外层保留 `asset_path`，内层匹配结果中剔除 `filePath`）。
    *   在外层增加了命中的敏感信息与特征类型列表 `captured_types`（例如：`["secrets", "crypto"]`），方便下游系统根据类型直接做路由或过滤。

---

## 4. 系统边界与外部交互
JXScout 作为一个中间件工具，其系统边界主要包含以下五个方面：
...
5. **NATS Message Queue (消息队列中间件)**:
   - 依赖 NATS 消息队列实例。当启用 NATS 功能时，JXScout 将会在每次 AST 分析任务完成时，通过 NATS 协议向指定的 Subject 推送格式化的分析结果 Payload。
